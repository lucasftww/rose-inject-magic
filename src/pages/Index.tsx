...
const fetchAllValorantSkins = async (): Promise<Map<string, { name: string; image: string }>> => {
  ...
  for (const s of (data.data || [])) {
    ...
    if (image && s.uuid) {
      map.set(s.uuid.toLowerCase(), { name: s.displayName, image });
    }
  }
  return map;
};
...
const FloatingWidgets = () => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      ...
      <a
        href="https://www.instagram.com/royalstorebr.ofc/"
        target="_blank"
        rel="noopener noreferrer"
        className="flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-[#f09433] via-[#e6683c] to-[#dc2743] text-white shadow-[0_0_20px_rgba(225,48,108,0.4)] transition-all hover:scale-110 hover:shadow-[0_0_30px_rgba(225,48,108,0.6)]"
      >
          ...
          <div className="grid grid-cols-1 gap-12 md:grid-cols-4">
            <div className="md:col-span-1">
              ...
              <div className="mt-4 flex gap-3">
                <a href="#" className="flex h-9 w-9 items-center justify-center rounded-full border border-border text-muted-foreground transition-colors hover:border-success hover:text-success">
