...
const LztContaCard = ({ item, skinsMap, formatPrice }: { item: LztItem; skinsMap: Map<string, SkinEntry>; formatPrice: (price: number, currency?: string) => string }) => {
  ...
  return (
    ...
    >
      {/* Skin preview area - taller for more visual impact */}
      ...
      <div className="p-3.5 sm:p-5 flex flex-col flex-1 gap-3 sm:gap-4">
        {/* Feature list - compact and clean */}
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center gap-1.5">
            <div className="flex h-4 w-4 items-center justify-center rounded-full bg-success/15 flex-shrink-0">
              <svg className="h-2.5 w-2.5 text-success" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>
            </div>
        ...
        <div className="mt-auto pt-2 sm:pt-3 border-t border-border/30">
          <p className="text-lg sm:text-xl font-bold text-success tracking-tight">
    ...
    <section className="border-t border-border bg-background px-5 sm:px-6 py-14 sm:py-24">
      <div className="mx-auto max-w-7xl">
        ...
        {loadingAccounts ? (
          ...
          <motion.div
            className="mt-8 sm:mt-14 grid grid-cols-2 gap-2.5 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3"
            initial="hidden"
            whileInView="visible"
    ...
    <div className="relative min-h-screen bg-background overflow-x-hidden">
      ...
      <main className="relative flex min-h-[85vh] sm:min-h-screen flex-col items-center justify-center px-6 sm:px-6 pt-4 sm:pt-8 pb-10 sm:pb-20 text-center overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.8, ease: [0.22, 1, 0.36, 1] }} className="z-10 mt-8 sm:mt-12 flex w-full max-w-md sm:max-w-none sm:w-auto flex-col sm:flex-row items-center justify-center gap-3 sm:gap-5 mx-auto">
          ...
          <Link to="/contas" className="w-full sm:w-auto border-2 border-success/40 px-10 py-3.5 sm:py-4 text-sm sm:text-base font-medium text-success transition-colors hover:border-foreground hover:text-foreground rounded-xl sm:rounded-none text-center">
            Ver Contas
          </Link>
        </motion.div>
